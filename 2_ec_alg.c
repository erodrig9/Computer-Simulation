...
#define JJJYEAH           // Define load balancing policy (SHORT, RR, RAND, JJJYEAH)
...
void sim(int argc, char *argv[])
{
  ...
  One = table("queue 1 service");
  Two = table("queue 1 service");
  Three = table("queue 1 service");
  Four = table("queue 1 service");
  Five = table("queue 1 service");
...
void load_balancer (double org_time, double service_time)
{
...
#if (defined(SHORT) && defined(DELAY_OFF))
...
#elif defined(JJJYEAH)
  Queue_len[0] = table_sum(One);
  Queue_len[1] = table_sum(Two);
  Queue_len[2] = table_sum(Three);
  Queue_len[3] = table_sum(Four);
  Queue_len[4] = table_sum(Five);
#endif

#if (defined(SHORT) || defined(JJJYEAH))
...
#if (defined(RAND) || defined(SHORTSS) || defined(JJJYEAH))
  ...
  if(num_ties > 1)
  {
    serv_util[0] = table_sum(One);
    serv_util[1] = table_sum(Two);
    serv_util[2] = table_sum(Three);
    serv_util[3] = table_sum(Four);
    serv_util[4] = table_sum(Five);
    
    jigga = serv_util[ties[0]];
    for(i=1; i<num_ties; i++)
    {
      if(serv_util[ties[i]] < jigga)
        jigga = serv_util[ties[i]];
    }

    if(jigga > (double)0)
    {

      if(jigga == serv_util[0])
        Select_q = 0;
      else if(jigga == serv_util[1])
        Select_q = 1;
      else if(jigga == serv_util[2])
        Select_q = 2;
      else if(jigga == serv_util[3])
        Select_q = 3;
      else
        Select_q = 4;
    }
...
#if (defined(RAND) || defined(SHORT) || defined(JJJYEAH))
